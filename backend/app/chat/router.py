...
Tracing = Annotated[observability.TracerProvider | None, Depends(get_tracer_provider)]
...
async def chat(
    ...
    settings: Config,
    tracer_provider: Tracing,
) -> AssistantTransportResponse:
    async def run(controller: RunController) -> None:
        await service.run_callback(controller, data, user, pool, client, settings, tracer_provider)
